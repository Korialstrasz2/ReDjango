...
**Special rule — authorial freedom, open catalog queries, and ReDjango legality.** The agent is explicitly free to improve a Unit rather than merely transcribe Elder Django. For a non-humanoid, the Elder `SkillNpc` list is a baseline, not a ceiling: the agent may propose additional innate `skill_actions` when they are strongly supported by the Unit’s lore, biology or supernatural nature, expected combat behavior, and comparison with related Units. This freedom is always subordinate to ReDjango’s implemented rules. Every cost, resource, variable, damage type, resistance, status, range, target pattern, duration, trigger, and effect must be found in—or be expressible entirely through—the current ReDjango rule vocabulary and handlers. At the time this guide was verified, `backend.combat.rules.DAMAGE_TYPES` permits only `Contundente`, `Perforante`, `Taglio`, `Gelo`, `Fuoco`, `Elettro`, and `Puro`; `Veleno` may be an existing effect/status, but “poison damage” is not a damage type. The agent must query this registry again at conversion time and must never invent mechanics that merely sound plausible, such as “Strength damage,” “poison damage,” a new resistance, a new status, or an unsupported resource; it must query the current game-variable registry, damage rules, effects/status vocabulary, action schema, and comparable implemented Skills, cite the matching rule in the dossier, and block the proposal if no legal representation exists. Even a legacy action name such as `Sputo Velenoso` proves only identity: its mechanics must be re-authored with supported damage/effect primitives, not copied as unsupported poison damage. The system may openly query the complete current Skill and item catalogs and choose any catalog-valid candidates that fit the identity brief: humanoids may receive curated current Skill and equipment pools, while creatures may use those catalogs as mechanical examples but still receive no `SkillPersonaggio` records or equipment—creature capabilities must be authored as legal innate actions. Catalog availability is permission to consider a candidate, not evidence that it belongs to the Unit; lore fit, type expectations, compatibility, prerequisites, level suitability, and family coherence remain mandatory.
...
| Rule vocabulary | every damage type, status, variable, resource, and effect resolves to a current implemented rule |
