...
type DetailTab = "player" | "management";
type ManagementView = "profile" | "edit";
...
function hasPositiveCosts(costs: ActiveReminder["costs"]): boolean {
  return Object.values(costs || {}).some((value) => Number(value) > 0);
}

function SkillRuleDisclosure({ title, meta, symbol, theme, defaultOpen = false, children }: {
  title: string;
  meta?: string;
  symbol: string;
  theme: "arcane" | "gold";
  ...
  return <details className="skill-rule-disclosure" open={open} onToggle={(event) => setOpen(event.currentTarget.open)} data-component-type="disclosure" data-theme={theme}>
    <summary><span className="skill-rule-symbol" aria-hidden="true">{symbol}</span><span><strong>{title}</strong>{meta && <small>{meta}</small>}</span><span className="skill-rule-chevron" aria-hidden="true">⌄</span></summary>
    <div className="skill-rule-body">{children}</div>
  </details>;
}
...
function SkillDetailModal({ skill, catalog, saving, onClose, onUnlock, onSave, onArchive, onDelete }: {
  ...
  const [tab, setTab] = useState<DetailTab>("player");
  const [managementView, setManagementView] = useState<ManagementView>("profile");
  ...
  const normalizedRequirements = skill.requirementsText.trim().toLocaleLowerCase("it-IT").replace(/\s+/g, " ");
  const normalizedPrerequisites = prerequisiteNames.join(", ").trim().toLocaleLowerCase("it-IT").replace(/\s+/g, " ");
  const showRequirementsText = Boolean(normalizedRequirements && normalizedRequirements !== normalizedPrerequisites);
  ...
  const hasCompletePricingModifier = pricingModifier?.amount !== undefined && pricingModifier.minimumBaseCost !== undefined && Boolean(managedDiscountTypes);
  const profileEntries = Object.entries(skill.profileTags || {});
  const hasPrimaryPlayerCopy = Boolean(skill.description.trim() || skill.rulesCost.trim() || showRequirementsText || skill.notes.trim() || skill.spell);
  const editing = tab === "management" && managementView === "edit";
  const blockedReasons = skill.unlock.owned ? [] : skill.unlock.blockedReasons;
  return <Modal surface="skills-detail" title={skill.name} onClose={onClose} wide footer={!editing && <><div className="skill-detail-footer-copy"><span className="skill-detail-status">{skill.unlock.owned ? "Abilità posseduta" : `${skill.xpCost} PE ${skill.xpTypeLabel}`}</span>{blockedReasons.length > 0 && <aside className="skill-detail-footer-warning" role="status"><strong>Avvertenze:</strong>{blockedReasons.map((reason) => <span key={reason}>{reason}</span>)}</aside>}</div><button className="button primary" onClick={onUnlock}>{skill.unlock.owned ? "Già sbloccata" : "Sblocca abilità"}</button></>}>
    <article className="skill-detail-card" data-component-type="card" data-theme={skill.magic ? "arcane" : "parchment"} data-spell-tier={skill.spell?.tier || "none"}>
      <header className="skill-detail-heading"><p className="eyebrow">{skill.familyGroup} · {skill.familyName}</p><div className="skill-seal" aria-hidden="true">{skill.magic ? "✧" : "✦"}</div></header>
      {catalog.permissions.canManageSkills && <nav className="skill-card-tabs" data-component-type="tabset" data-theme="gold" aria-label="Viste dell'abilità"><button role="tab" aria-selected={tab === "player"} className={tab === "player" ? "active" : ""} onClick={() => setTab("player")}>Giocatore</button><button role="tab" aria-selected={tab === "management"} className={tab === "management" ? "active" : ""} onClick={() => setTab("management")}>Gestione</button></nav>}
      {tab === "player" && <section className="skill-player-document" data-component-type="document" data-theme={skill.magic ? "arcane" : "parchment"}>
        <dl className="skill-document-facts"><div><dt>Costo</dt><dd>{skill.xpCost} PE {skill.xpTypeLabel}</dd></div>{prerequisiteNames.length > 0 && <div><dt>Prerequisiti</dt><dd>{prerequisiteNames.join(", ")}</dd></div>}{skill.pricing.ownedSkillDiscount > 0 && <div><dt>Sconto automatico</dt><dd>-{skill.pricing.ownedSkillDiscount} PE · {skill.pricing.ownedSkillDiscountSources.join(", ")}<small>Rimuovendo la skill che concede lo sconto, questo prezzo torna a {skill.pricing.calculatedBeforeOwnedSkillDiscount} PE.</small></dd></div>}</dl>
        {skill.description.trim() && <section className="skill-document-section"><p className="eyebrow">Descrizione</p><p>{skill.description}</p></section>}
        {skill.rulesCost.trim() && <aside className="skill-document-callout"><strong>Costo d'uso</strong><p>{skill.rulesCost}</p></aside>}
        {showRequirementsText && <aside className="skill-document-callout"><strong>Requisiti</strong><p>{skill.requirementsText}</p></aside>}
        {skill.notes.trim() && <aside className="skill-document-note"><strong>Nota</strong><p>{skill.notes}</p></aside>}
        {hasCompletePricingModifier && <aside className="skill-document-callout"><strong>Regola automatica del sistema</strong><p>Quando questa skill è sbloccata, il sistema sottrae {String(pricingModifier?.amount)} PE alle skill {managedDiscountTypes} con costo base di almeno {String(pricingModifier?.minimumBaseCost)} PE. Se la skill viene rimossa, lo sconto scompare e i prezzi tornano normali.</p></aside>}
        {skill.spell && <section className="skill-document-section skill-spell-document" data-component-type="panel" data-theme="arcane"><header><div><p className="eyebrow">Incantesimo</p><h3>{skill.spell.tierLabel}</h3></div>{skill.spell.range.trim() && <span><small>Raggio</small>{skill.spell.range}</span>}</header>{skill.spell.costSummary.trim() && <p className="skill-spell-cost-summary">{skill.spell.costSummary}</p>}{skill.spell.costNotes.trim() && <aside><strong>Note sui costi</strong><p>{skill.spell.costNotes}</p></aside>}<SpellCalculator skill={skill} characterId={catalog.character?.id || null} /></section>}
        {skill.passiveEffects.length > 0 && <section className="skill-document-section"><header className="skill-document-section-heading"><div><p className="eyebrow">Sempre attivi</p><h3>Effetti passivi</h3></div><span>{skill.passiveEffects.length}</span></header><div className="skill-rule-list">{skill.passiveEffects.map((passive, index) => <SkillRuleDisclosure key={passive.id} title={passive.name} meta={skill.unlock.acceptedPassiveIds.includes(passive.id) ? "Accettato e applicato" : "Proposto allo sblocco"} symbol="✧" theme="arcane" defaultOpen={!hasPrimaryPlayerCopy && skill.activeReminders.length === 0 && index === 0}>{passive.description.trim() && <p>{passive.description}</p>}{passive.operations.length > 0 && <ul>{passive.operations.map((operation, operationIndex) => <li key={operationIndex}>{operationLabel(operation)}</li>)}</ul>}</SkillRuleDisclosure>)}</div></section>}
        {skill.activeReminders.length > 0 && <section className="skill-document-section"><header className="skill-document-section-heading"><div><p className="eyebrow">Da dichiarare</p><h3>Azioni</h3></div><span>{skill.activeReminders.length}</span></header><div className="skill-rule-list">{skill.activeReminders.map((action, index) => <SkillRuleDisclosure key={action.id} title={action.name} meta={hasPositiveCosts(action.costs) ? costsLabel(action.costs) : undefined} symbol="✦" theme="gold" defaultOpen={!hasPrimaryPlayerCopy && index === 0}>{action.description.trim() && <p>{action.description}</p>}{(action.trigger.trim() || action.duration.trim()) && <dl>{action.trigger.trim() && <div><dt>Innesco</dt><dd>{action.trigger}</dd></div>}{action.duration.trim() && <div><dt>Durata</dt><dd>{action.duration}</dd></div>}</dl>}{action.usageNotes.trim() && <aside><strong>Nota d'uso</strong><p>{action.usageNotes}</p></aside>}</SkillRuleDisclosure>)}</div></section>}
      </section>}
      {tab === "management" && catalog.permissions.canManageSkills && <section className="skill-management-document">
        {managementView === "profile" ? <><header className="skill-management-heading"><div><p className="eyebrow">Solo gestione</p><h3>Profilo e classificazione</h3></div><button type="button" className="button secondary" onClick={() => setManagementView("edit")}>Modifica abilità</button></header><dl className="skill-management-facts"><div><dt>Record</dt><dd>#{skill.number}</dd></div><div><dt>Stato</dt><dd>{skill.archived ? "Archiviata" : "Attiva"}</dd></div></dl>{profileEntries.length > 0 && <div className="skill-tag-cloud">{profileEntries.map(([key, value]) => <span key={key}><strong>{key}</strong>{Array.isArray(value) ? value.join(", ") : String(value)}</span>)}</div>}{skill.profileNotes.trim() && <section className="skill-management-notes"><h4>Note profilo</h4><p>{skill.profileNotes}</p></section>}</> : <section className="skill-edit-tab"><SkillEditor key={skill.id} skill={skill} families={catalog.families} skillOptions={catalog.skillOptions} effectConfiguration={catalog.effectConfiguration} saving={saving} onCancel={() => setManagementView("profile")} onSave={onSave} onArchive={onArchive} onDelete={onDelete} /></section>}
      </section>}
    </article>
  </Modal>;
}
