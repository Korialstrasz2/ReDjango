...
function SkillDetailModal({ skill, catalog, saving, onClose, onUnlock, onSave, onArchive, onDelete }: {
  ...
  const hasPrimaryPlayerCopy = Boolean(skill.description.trim() || skill.rulesCost.trim() || skill.requirementsText.trim() || skill.notes.trim() || skill.spell);
  const editing = tab === "management" && managementView === "edit";
  return <Modal surface="skills-detail" title={skill.name} onClose={onClose} wide footer={!editing && <><span className="skill-detail-status">{skill.unlock.owned ? "Abilità posseduta" : `${skill.xpCost} PE ${skill.xpTypeLabel}`}</span><button className="button primary" onClick={onUnlock}>{skill.unlock.owned ? "Già sbloccata" : "Sblocca abilità"}</button></>}>
    <article className="skill-detail-card" data-component-type="card" data-theme={skill.magic ? "arcane" : "parchment"} data-spell-tier={skill.spell?.tier || "none"}>
      <header className="skill-detail-heading"><div><p className="eyebrow">{skill.familyGroup} · {skill.familyName}</p><h3>{skill.name}</h3></div><div className="skill-seal" aria-hidden="true">{skill.magic ? "✧" : "✦"}</div></header>
      ...
      {tab === "player" && <section className="skill-player-document" data-component-type="document" data-theme={skill.magic ? "arcane" : "parchment"}>
        ...
        {skill.requirementsText.trim() && <aside className="skill-document-callout"><strong>Requisiti</strong><p>{skill.requirementsText}</p></aside>}
        ...
        {skill.unlock.blockedReasons.length > 0 && !skill.unlock.owned && <aside className="skill-blocked-reasons skill-document-callout"><strong>Avvertenze</strong><ul>{skill.unlock.blockedReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></aside>}
      </section>}
