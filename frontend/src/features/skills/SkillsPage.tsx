...
function ReminderModal({ reminder, onClose }: { reminder: ActiveReminder; onClose: () => void }) {
  return <Modal title={reminder.name} onClose={onClose} footer={<button className="button primary" onClick={onClose}>Ho letto</button>}>
...
function SpellCalculator({ skill, characterId }: { skill: UnifiedSkill; characterId: number | null }) {
  ...
  return <section className="spell-calculator" data-spell-tier={skill.spell.tier}>
    ...
    {result && <div className="spell-resource-options"><article><small>Mana</small><strong>{result.resourceOptions.mana}</strong></article><article><small>Energia alternativa</small><strong>{result.resourceOptions.energy ?? "—"}</strong></article><article><small>PA alternativi</small><strong>{result.resourceOptions.actionPoints ?? "—"}</strong></article><article><small>{result.effectUnit} risultante</small><strong>{result.projectedEffect}</strong></article><p>{result.note}</p></div>}
    <aside><strong>Predisposto per il combattimento</strong><p>La formula e le conversioni sono pronte; la futura sezione combattimento deciderà quale opzione applicare e quando spendere davvero le risorse.</p></aside>
  </section>;
}
...
function SkillDetailModal({ skill, catalog, saving, onClose, onUnlock, onSave, onArchive, onDelete }: {
  ...
  return <Modal title={skill.name} onClose={onClose} wide footer={tab !== "edit" && <><span className="skill-detail-status">{skill.unlock.owned ? "Abilità posseduta" : `${skill.xpCost} PE ${skill.xpTypeLabel}`}</span><button className="button primary" onClick={onUnlock}>{skill.unlock.owned ? "Già sbloccata" : "Sblocca abilità"}</button></>}>
...
function SkillCreationModal({ catalog, characterId, saving, onClose, onSave }: { catalog: UnifiedSkillCatalog; characterId: number | null; saving: boolean; onClose: () => void; onSave: (values: Record<string, unknown>) => void }) {
  ...
  return <Modal title="Crea abilità" onClose={onClose} wide>
...
function XpEditorModal({ catalog, saving, onClose, onSave }: {
  ...
  return <Modal title="Modifica Punti Esperienza" onClose={onClose} footer={<><button className="button secondary" disabled={saving} onClick={onClose}>Annulla</button><button className="button primary" disabled={saving} onClick={() => onSave(values)}>{saving ? "Salvataggio…" : "Salva PE disponibili"}</button></>}>
