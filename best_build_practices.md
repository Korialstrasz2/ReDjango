...
- The Rapido tab is a hover cascade with no submit button: race → culture → sex, each level nested inside the previous level's `<li>` so entering a flyout is not a mouse-leave. A click at any level generates immediately and the levels below it are rolled — `randomCulture` is what the race-only click sends, and the result declares what the dice decided. Because the open cascade needs roughly 480 px, this tool uses the wide drawer; the flyouts must stay inside the drawer, which is `overflow: hidden`.
- Elder's reference art is imported, not referenced in place. The sources are 1024×1024 PNGs of about 1.8 MB, so `import_legacy_race_media` converts them to WebP at quality 70 with the long edge capped at 640 px — 81 assets drop from roughly 145 MB to 3.2 MB. Filename matching is case-insensitive and understands the race aliases, because Elder mixes `orco-gen.png` with `Ashlander-m.png`.
- Portraits are ordinary `UploadedImage` rows with `usage_type="race_portrait"`. Clips are **not**: the image archive renders every row as `<img>`, so an mp4 stored there would show as a broken thumbnail. Video is a new media kind with its own `media_library.VideoClip` model, and `.mp4` belongs in both the inline and range extension sets so the browser plays it instead of downloading it.
- The race has no table of its own — it is a text column on `NomiRazzeInfo` — so its portrait is repeated on every culture of that race and the selector reads it from the first one. That is deliberate denormalisation chosen over adding a model for 17 rows.
- Clips are around 1 MB each, so the side panel swaps the image immediately but mounts the `<video>` only after the pointer has been still for `CLIP_SETTLE_MS`. Sweeping the list must not start a download per row. Missing assets resolve to an empty string and render a placeholder, never a broken image.
...
- The loop is bounded twice: the agent profile limits iterations, while `RunBudget` limits wall-clock time, aggregate tokens and tool calls. Provider timeouts inherit the remaining wall-clock budget and cancellation is checked before every provider/tool boundary.
...
- A provider is *ready* only when it is enabled and has every required endpoint, credential and model. Readiness is separate for chat and images, is exposed with concrete configuration issues, and is enforced again in services. An agent pinned to a broken provider never silently falls back to another one.
- Provider/agent defaults are database invariants: at most one live default per purpose and one live default agent. Save services clear the previous default in the same transaction; selectors then resolve only ready defaults.
- Model suggestions are only the offline fallback. Gestione AI can fetch the live `/models` catalogue (or the Stable Diffusion checkpoint list), persists the normalized result and its refresh time, and validates model-specific capabilities and context limits server-side. Secrets remain write-only and non-admin managers receive a redacted endpoint.
...
- Chat and image requests return a persisted `AIExecutionRun` with `202 Accepted`; the SPA polls its owner-only status endpoint and can request cancellation. The local executor never holds an HTTP request or a database transaction across a provider call. Stale run recovery prevents a server restart from blocking an account indefinitely.
- Conversation retention is deliberately small: `AIConversation` keeps only the three most recently touched conversations for each Django user, with at most 80 display bubbles per conversation. Failed or queued-then-cancelled empty conversations are deleted. Run records are operational state and terminal rows older than one day are cleaned on the next request.
...
- Model identifiers remain configurable strings; the live provider catalogue informs the picker and validation without turning a vendor's fast-changing model names into application constants.
