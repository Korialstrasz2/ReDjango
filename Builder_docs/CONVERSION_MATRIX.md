...
| AI/orchestration | prompts, logs, Comfy/TTS/orchestrator tools | `AIProvider`, `AIAgentProfile`; provider-neutral read-only tool registry | Seeded provider catalog upgraded conservatively plus default campaign agent | Bounded configurable agent runtime, role/tool enforcement, OpenAI Responses and Anthropic/compatible adapters; structured content-free run logs | Gestione AI edits competence, scope, permission, provider and autonomy; assistant selects a workflow rather than a raw provider | Provider payload, credential boundary, role/tool policy, bounded loop and OpenAI Responses contract tests | Core agentic slice complete. Image editing and ComfyUI remain explicitly WIP; no conversation or response logging in the database. |
