...
from .config import configuration_payload, get_generator_rules, get_market_locations, get_shop_type_definitions, market_settings_payload, resolve_location
...
def rollable_rarity_values() -> set[int]:
    """Rarities the generator can actually draw.

    A rarity with probability 0 excludes its items as completely as a missing
    lv_loot does, but the generator never says so. Computing it here is what
    turns that into a reported reason instead of an invisible one.
    """
    return {
        int(rarity)
        for rarity, probability in get_generator_rules()["rarityProbabilities"].items()
        if probability > 0
    }
